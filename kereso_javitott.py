...
def keres(katalogus, szerzo=None, cim=None, kiadas=None, case_sensitive_search=True):
    ...
    for elem in katalogus:
        if "katalogus" in elem:
            # Hibajavítás: a belső katalógus találatait hozzáadjuk
            belso_talalatok = keres(elem["katalogus"], szerzo, cim, kiadas, case_sensitive_search)
            talalatok.extend(belso_talalatok)
        elif "cim" in elem:
            # Keresés kis/nagybetű érzékenység figyelembevételével
            if not case_sensitive_search:
                elem_szerzo = elem["szerzo"].lower()
                elem_cim = elem["cim"].lower()
                szerzo_keres = None if szerzo is None else szerzo.lower()
                cim_keres = None if cim is None else cim.lower()
            else:
            ...
            if ((cim_keres is None or cim_keres in elem_cim) and
                (szerzo_keres is None or szerzo_keres in elem_szerzo) and
...
# Példa keresések
print("Címben 'Le' (alapértelmezett case-sensitive):")
print(keres(katalogus, cim="Le"))

print("\nCímben 'le' (case-sensitive = True):")
print(keres(katalogus, cim="le", case_sensitive_search=True))

print("\nCímben 'le' (case-sensitive = False):")
